...
const addPageImages = async (req, res) => {
    try {
        ...
        if (checkPageName) {
            ...
            return res.status(200).send({ status: false, message: 'Page image updated'})
        }
...
const getAllPageImages = async (req, res) => {
    try {
        ...
        if (pageImages.length === 0) {
            return res.status(404).send({ status: false, message: 'No page image found'})
        }
...
const getImageByPageName = async (req, res) => {
    try {
        ...
        if (!pageName) {
            return res.status(404).send({ status: false, message: 'No page image found with this page name'})
        }
